...
# Source: https://github.com/guptapraful/niqe/blob/master/niqe.py
# Author: Praful Gupta (UT Austin Graduate Student)
# Github: https://github.com/guptapraful
# License: No license (private use only!)
...
class NIQE(IProcess):
	...
	def aggd_features(self, imdata):
		...
		alpha = self.niqe_gamma_range[pos]
	...
	def ggd_features(self, imdata):
		...
		return self.niqe_gamma_range[pos], sigma_sq
	...
	def _get_patches_generic(self, img, patch_size, is_train, stride):
		...
		img2 = img2.resize((int(img2.width * 0.5), int(img2.height * 0.5)), resample = Image.BICUBIC)
		img2 = np.array(img2)
	...
	def __init__(self):
		...
		self.niqe_gamma_range = np.arange(0.2, 10, 0.001)
		a = scipy.special.gamma(2.0/self.niqe_gamma_range)
		a *= a
		b = scipy.special.gamma(1.0/self.niqe_gamma_range)
		c = scipy.special.gamma(3.0/self.niqe_gamma_range)
	...
	def do(self, imageData):
		...
		# TODO: memoize
		params = scipy.io.loadmat(join(module_path, 'include', 'niqe_image_params.mat'))
		...
		feats = self.get_patches_test_features(inputImgData, patch_size)
		...
		self.niqe_score = np.sqrt(np.dot(np.dot(X, pinvmat), X))
